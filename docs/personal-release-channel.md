...
自动更新不是发布时的服务端推送：开关默认关闭，开启后只在设置的每日时间执行；若当时有任意会话繁忙，本次会跳过并顺延到下一天。个人源码发行会重新运行受信安装目录中的 `install.sh`，从固定的 `lukecc00/botmux@p/ai_open` 构建新 release 并原子切换 `current`；开启「自动重启」后才会立即重启应用，否则等下次手动重启生效。
