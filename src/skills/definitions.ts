...
不要只在最后发结果。以下时机应主动发一条 \`--no-mention\` 过程卡：
- 完成一个可验证阶段；
- 开始预计较久的构建、测试或等待之前；
- 新发现改变了下一步方案；
- 出现有意义的风险或阻塞。
...
决策规则（**按是否结束 / 是否需要对方行动判断，不是按内容是否重要**）：
- **整轮对话结束，或明确需要对方确认 / 决策 / 授权 / 补充信息 / 处理阻塞** → \`--mention-back\`（@回触发者）或 \`--mention\` 点名，确保对方看到。
- **过程更新 / 阶段结论 / 状态记录 / 简短确认（"收到""在看"）** → \`--no-mention\`，即使内容重要也不要每轮 @ 人。
- **如果只是没信息量的"收到"** → 不如不发，等下一条有内容时再回。
- ⚠️ 过程消息默认不 @；不要因为“有实质结论”就在任务尚未结束时打扰人。
