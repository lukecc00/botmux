...
function routeHasNarrowUntrustedAuth(method: string, pathname: string): boolean {
  ...
  if (method === 'POST' && /^\/api\/sessions\/[^/]+\/(?:slash|cd|close|preview|chat-rename|progress-card|project|project-dispatch-policy)$/.test(pathname)) return true;
...
ipcRoute('PUT', '/api/bot-card-prefs', async (req, res) => {
  ...
  if (body.topicGroupMemory && typeof body.topicGroupMemory === 'object' && !Array.isArray(body.topicGroupMemory)) {
    ...
    if (raw.provider === 'auto' || raw.provider === 'local' || raw.provider === 'tencentdb') memory.provider = raw.provider;
    ...
    if (typeof raw.maxPromptChars === 'number' && Number.isInteger(raw.maxPromptChars) && raw.maxPromptChars >= 500 && raw.maxPromptChars <= 8_000) memory.maxPromptChars = raw.maxPromptChars;
    if (typeof raw.maxSummaryChars === 'number' && Number.isInteger(raw.maxSummaryChars) && raw.maxSummaryChars >= 500 && raw.maxSummaryChars <= 10_000) memory.maxSummaryChars = raw.maxSummaryChars;
    ...
    if (raw.tencentdb && typeof raw.tencentdb === 'object' && !Array.isArray(raw.tencentdb)) {
      const input = raw.tencentdb as Record<string, unknown>;
      const tencentdb: import('../bot-registry.js').TopicGroupMemoryTencentDbConfig = {};
      for (const key of ['runtimeDir', 'endpoint', 'apiKey', 'serviceId', 'teamId', 'agentId', 'userId', 'panelUrl'] as const) {
        if (typeof input[key] === 'string') tencentdb[key] = input[key].trim();
      }
      if (typeof input.maxResults === 'number' && Number.isInteger(input.maxResults) && input.maxResults >= 1 && input.maxResults <= 20) tencentdb.maxResults = input.maxResults;
      if (typeof input.includePersona === 'boolean') tencentdb.includePersona = input.includePersona;
      if (typeof input.includeScenes === 'boolean') tencentdb.includeScenes = input.includeScenes;
      if (typeof input.timeoutMs === 'number' && Number.isInteger(input.timeoutMs) && input.timeoutMs >= 1_000 && input.timeoutMs <= 60_000) tencentdb.timeoutMs = input.timeoutMs;
      if (Object.keys(tencentdb).length > 0) memory.tencentdb = tencentdb;
    }
