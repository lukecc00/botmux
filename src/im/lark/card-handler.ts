...
export async function handleCardAction(data: CardActionData, deps: CardHandlerDeps, larkAppId?: string): Promise<any> {
  ...
  const isSensitive = value?.action && ['restart', 'close', 'resume', 'skip_repo', 'repo_manual_submit', 'repo_worktree_submit', 'worktree_toggle_mode', 'retry_last_task', 'retry_turn', 'get_write_link', 'manage_access', 'open_local_terminal', 'open_local_cli', 'toggle_stream', 'toggle_display', 'export_text', 'term_action', 'refresh_screenshot', 'takeover', 'disconnect', 'tui_keys', 'tui_text_input', 'wf_approve', 'wf_reject', 'wf_cancel', 'stop_turn', 'compact_session', 'quote_confirm'].includes(value.action);
  if (isSensitive) {
  ...
  if (value?.action) {
    ...
    if (actionType === 'close') {
      ...
      // Preserve the personal release's high-attention close callback. The
      // provider UUID makes this idempotent if worker-exit and card-close race.
      await notifySessionStopped(current, deps.sessionReply, 'ended', {
        recipientOpenId: operatorOpenId,
      });
    ...
    if (actionType === 'manage_access') {
      ...
      // Both links are privileged. Deliver only to the verified callback
      // operator (ephemeral in normal groups, DM fallback elsewhere).
      void deliverWriteLinkCard(ds, operatorOpenId, cardJson);
