...
命令:
  ...
  memory help          显示话题群共享记忆专项帮助
  memory status [--json]
                       查看当前 key 的 revision、更新时间、文件大小和条目统计
  memory list [--lark-app-id <app>]
                       列出当前 bot（或指定 app）的所有话题群 memory
  memory read [--json] / path / compact
                       读取、定位或确定性压缩当前话题群 memory
  memory append [--kind summary|fact|decision|question|contribution|resource] <text>
                       手动追加/更新；正文可来自位置参数、stdin 或 --content-file
  memory clear --yes   删除当前群 memory（不会关闭或删除 session）
