...
export const messages: Record<string, string> = {
  ...
  'ai.identity.mention_gate': '- **@ hard-gate**: every `botmux send` MUST explicitly pick one or it errors out — `--mention` (name someone) / `--mention-back` (@ the triggerer) / `--no-mention` (none). Progress updates, stage conclusions, and status records always use --no-mention, even when important; do not @ a person on every turn. Use --mention-back only when the conversation is ending or the recipient must confirm, decide, authorize, provide information, or clear a blocker (or use --mention to name someone). A contentless "got it" is better not sent.',
  ...
  'ai.shell.codex_structured_delivery': 'Codex delivery rule: every user-facing commentary/progress message and the final answer is mirrored automatically as a separate in-thread Lark Markdown card. During long tasks, write a self-contained commentary immediately after each verifiable stage, before a long build/test/wait, and whenever a new finding changes the next step; do not wait for the final answer or merge distinct milestones. In a “stage result → tool call → next-stage note → long command/wait” sequence, write both notes as separate commentary messages: tool calls, command output, Updated Plan, and the final answer cannot replace or absorb either note. For each ordinary progress update, write the commentary first and then call `botmux send --no-mention` with exactly the same body; botmux idempotently merges both paths by session, turn, and content, preserving the native progress card with Web Terminal controls. Put the final answer only in final and do not explicitly resend the same answer; the final card handles user notification. Continue to use `botmux send` as needed for attachments, cross-chat delivery, or naming another person/bot.',
  'ai.shell.mention_gate': '@ decision (mandatory): every `botmux send` MUST explicitly pick one or it errors — `--mention <open_id:name>` (name a person/bot; REQUIRED to communicate or collaborate with another bot) / `--mention-back` (@ the sender of the message you are replying to) / `--no-mention` (none). Progress updates, stage conclusions, and status records always use `--no-mention`, even when important; do not @ a person on every turn. Use `--mention-back` only when the conversation is ending or the recipient must confirm, decide, authorize, provide information, or clear a blocker. Use `--mention` to name another person/bot. A contentless "got it" is better not sent.',
  ...
  'ai.followup.codex_structured_delivery': 'Write key stages as user-facing commentary and finish with a final answer; both are mirrored automatically to in-thread Lark cards. Every commentary and the final answer becomes a separate card. For long tasks, write a self-contained commentary after each verifiable stage, before a long build/test/wait, and whenever a new finding changes the next step; do not wait for the final answer or merge milestones. In commentary → tool → commentary → tool/wait sequences, both commentary messages must be emitted separately; a later tool or final cannot replace the earlier one. After each ordinary commentary, call `botmux send --no-mention` with the same body; botmux merges the duplicate and preserves the native progress-card style. Put the final answer only in final. @ the user only when ending or when they must act, never on every update.',
