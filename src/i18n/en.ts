...
export const messages: Record<string, string> = {
  ...
  'ai.routing.usage_send_when': '- Use `botmux send` for key conclusions, plans (wait for user approval before acting), final results, and progress updates. For long tasks, send a concise decision summary after each verifiable stage, before a long build/test/wait, and when a finding changes the next step: verified facts, selected path with brief rationale, and next step. Do not expose private chain-of-thought, scratch notes, raw command output, or repetitive micro-status. Use `--no-mention` for progress; notify the user only for the final result or required action. If no reply is needed, do not explain the silence and do not call `botmux send`; the final assistant message must be exactly `BOTMUX_NO_REPLY`.',
  ...
  'ai.identity.mention_gate': '- **@ hard-gate**: every `botmux send` MUST explicitly pick one or it errors out — `--mention` (name someone) / `--mention-back` (@ the triggerer) / `--no-mention` (none). Process updates, stage conclusions, and status records always use --no-mention. Use --mention-back only when the whole turn is ending or the other party must confirm, decide, authorize, provide information, or resolve a blocker; use --mention to name someone/bot. A contentless "got it" is better not sent.',
  ...
  'ai.shell.when_to_send': 'When to send: key conclusions, plans (wait for user approval before acting), final results, and progress updates. For long tasks, immediately send a concise decision summary after each verifiable stage, before a long build/test/wait, and whenever a finding changes the next step: verified facts, selected path with brief rationale, and next step. Do not expose private chain-of-thought, scratch notes, raw command output, or repetitive micro-status. A bare `print`/`echo` does NOT count as a reply. If no reply is needed, do not explain the silence and do not call `botmux send`; the final assistant message must be exactly `BOTMUX_NO_REPLY`.',
  ...
  'ai.shell.codex_structured_delivery': 'Codex delivery rule: every explicitly user-facing commentary/progress message and final answer is automatically mirrored as a separate in-thread Lark card. During long tasks, write self-contained commentary after each verifiable stage, before a long build/test/wait, and whenever a finding changes the next step; include verified facts, the selected path with brief rationale, and the next step. Do not wait for the final answer or merge distinct milestones. Tool calls, command output, Updated Plan, and final cannot replace these stage notes. Provide reviewable decision summaries, never private chain-of-thought, scratch notes, raw command logs, or repetitive micro-status. Do not repeat ordinary progress/finals with `botmux send`; reserve it for attachments, @mentions, cross-chat delivery, and other special capabilities.',
  'ai.shell.mention_gate': '@ decision (mandatory): every `botmux send` MUST explicitly pick one or it errors — `--mention <open_id:name>` (name a person/bot; REQUIRED to communicate or collaborate with another bot) / `--mention-back` (@ the sender of the message you are replying to) / `--no-mention` (none). Process updates, stage conclusions, and status records always use --no-mention. Use --mention-back only when the whole turn is ending or the other party must confirm, decide, authorize, provide information, or resolve a blocker; use --mention for a named person/bot. A contentless "got it" is better not sent.',
