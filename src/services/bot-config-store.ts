...
export const CONFIG_FIELDS: readonly ConfigFieldSpec[] = [
  ...
  { key: 'stageConclusionCards', configKey: 'stageConclusionCards', kind: 'boolean', effect: 'immediate', clearable: false, hint: '阶段结论控制卡 on|off（默认 off）：把 CLI 已明确输出给用户的阶段结论/进度说明显示为带 Web 终端、停止、管理按钮的独立卡片；不会暴露隐藏思维链' },
