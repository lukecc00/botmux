...
export interface TopicGroupMemoryTencentDbConfig {
  ...
  serviceId?: string;
  /** v3 isolation templates expanded per request. */
  teamId?: string;
  agentId?: string;
  userId?: string;
...
function normalizeTopicGroupMemoryConfig(raw: unknown): TopicGroupMemoryConfig | undefined {
  ...
  if (input.injectMode === 'off' || input.injectMode === 'summary' || input.injectMode === 'summary-and-facts') out.injectMode = input.injectMode;
  if (input.updateMode === 'off' || input.updateMode === 'manual' || input.updateMode === 'auto') out.updateMode = input.updateMode;
  ...
  if (input.httpLlm && typeof input.httpLlm === 'object' && !Array.isArray(input.httpLlm)) {
    const value = input.httpLlm as Record<string, unknown>;
    const http: TopicGroupMemoryHttpLlmConfig = {};
    if (typeof value.enabled === 'boolean') http.enabled = value.enabled;
    if (typeof value.autoDiscoverCodex === 'boolean') http.autoDiscoverCodex = value.autoDiscoverCodex;
    if (typeof value.baseUrl === 'string' && value.baseUrl.trim()) http.baseUrl = value.baseUrl.trim();
    if (typeof value.model === 'string' && value.model.trim()) http.model = value.model.trim();
    if (value.api === 'auto' || value.api === 'responses' || value.api === 'chat-completions') http.api = value.api;
    if (typeof value.timeoutMs === 'number' && Number.isInteger(value.timeoutMs) && value.timeoutMs > 0) {
      http.timeoutMs = Math.min(value.timeoutMs, 300_000);
    }
    if (Object.keys(http).length > 0) out.httpLlm = http;
  }
  if (input.tencentdb && typeof input.tencentdb === 'object' && !Array.isArray(input.tencentdb)) {
    const value = input.tencentdb as Record<string, unknown>;
    ...
    for (const key of ['runtimeDir', 'endpoint', 'apiKey', 'serviceId', 'teamId', 'agentId', 'userId', 'panelUrl'] as const) {
      if (typeof value[key] === 'string' && value[key].trim()) tencentdb[key] = value[key].trim();
    }
    if (typeof value.maxResults === 'number' && Number.isInteger(value.maxResults) && value.maxResults > 0) {
      tencentdb.maxResults = Math.min(value.maxResults, 20);
    }
    if (typeof value.includePersona === 'boolean') tencentdb.includePersona = value.includePersona;
    if (typeof value.includeScenes === 'boolean') tencentdb.includeScenes = value.includeScenes;
    if (typeof value.timeoutMs === 'number' && Number.isInteger(value.timeoutMs) && value.timeoutMs > 0) {
      tencentdb.timeoutMs = Math.min(value.timeoutMs, 60_000);
    }
    if (Object.keys(tencentdb).length > 0) out.tencentdb = tencentdb;
  }
