...
The table lists the current built-in adapters (the **authoritative source** for `cliId`s is [`src/adapters/cli/registry.ts`](../../../src/adapters/cli/registry.ts), which changes across versions):
